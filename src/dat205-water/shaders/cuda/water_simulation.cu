...
RT_FUNCTION float3 gravity_force(float particle_density) {
  ...
  return make_float3(0.0f, g, 0.0f); // TODO: remove me
}
...
RT_PROGRAM void update() {
    ...
    if (nn_count > 0) {
      // rtPrintf("Particle %d has %d neighbors.\n", launch_index, nn_count);
    }
    ...
    // External forces
    float3 tot_force = gravity_force(p.density);
    ...
    for (int i = 0; i < nn_count; i++) {
      Particle& pi = particles_buffer[nn[i]];
      float3 away_vec = p.position - pi.position;
      float dist      = optix::length(away_vec);

      // if (40.0f * SUPPORT_RADIUS < dist) {
      //   continue;
      // }

      // p.velocity += 100.0f * dt * away_vec / (dist * dist);
