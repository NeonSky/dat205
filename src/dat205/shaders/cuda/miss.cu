...
RT_PROGRAM void miss_environment_constant() {
  ...
  float phi = 0.5f * M_PIf - acosf( ray.direction.y );
  ...
  // Artificially brighten the image a bit.
  float3 offset = make_float3(0.1f, 0.1f, 0.1f);

  payload.radiance = make_float3(tex2D(env_map, u, v)) + offset;
}
